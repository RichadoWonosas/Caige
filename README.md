...
- 支持“猜歌吃鸡”和“出你字母”两种模式。
- 主持人状态板直接展示答案与来源；玩家战况支持文字与图片两种紧凑输出。
- 主持人使用“猜对 / 猜错”按钮判定整曲回答。
- 支持 Unicode 字符分类、Alias 归一化、字符强制显示或隐藏。
- 支持题目乱序、恢复顺序、误操作撤销和完整行动记录。
- 可复制文字战况、复制或保存战况图，并导入、导出 JSON 存档。
- 支持已猜中题目的延迟隐藏及终局恢复；猜歌吃鸡终局会揭示胜者题目并标记胜者。
- 支持亮色、暗色、跟随系统及可调色相主题。
...
Caige 不需要服务器数据库。对局和偏好通过浏览器存储保留在本机；只有主持人主动导出存档、文字战况或战况图时，数据才会离开应用。
